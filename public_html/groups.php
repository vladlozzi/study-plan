...
if (!defined("IN_ADMIN")) { echo "<p style=\"text-align: center; color: red; font-weight: bold;\">".
                               "Помилка входу в модуль groups.php</p>"; require "footer.php"; exit(); }
...
$GQuery = "
				SELECT a.id, a.shufr_fak, b.fakultet_shufr, a.num_kurs, a.nazva_grupu, 
					a.plan_id, c.reg_number AS work_edu_plan 
...
?><br>
<table class="scrolling-table" style="margin-left: 0%;">
<thead>
...
if (!empty($_POST['addg'])) {
	...
	// перевірка, чи вже є така група
		...
		$query_result = mysqli_query($conn,  $GTestQuery) or 
					die("<tr><td colspan=5>Помилка сервера при запиті<br>".$GTestQuery.
		...
		if ($ig == 0) {
		   ...
		   $query_result = mysqli_query($conn, $AddGQuery) or 
						die("<tr><td colspan=10>Помилка сервера при запиті<br>".$AddGQuery.
							" : ".mysqli_error($conn)."</td></tr></table>");
?>
<tr><td colspan=6 style="color: green; font-weight: normal;">
...
$query_result = mysqli_query($conn, $GQuery) or die("Помилка сервера при запиті<br>".$GQuery." : ".mysqli_error($conn));
...
// $GQuery = "SELECT * FROM catalogGroup ORDER BY id";
$query2_result = mysqli_query($conn, $GQuery) or die("Помилка сервера при запиті<br>".$GQuery." : ".mysqli_error($conn));
...
//		echo " | id=".$query2_row['id'];
		...
		$GUQuery_result = mysqli_query($conn,  $GUQuery) or 
					die("<tr><td colspan=5>Помилка сервера при запиті<br>".$GUQuery.
							" : ".mysqli_error($conn)."</td></tr></table>");
...
//		echo " | "."ig=".$ig;
		if ($ig == 0) {
			...
			$UpdateGQuery_result = mysqli_query($conn,  $UpdateGQuery) or 
								die("<tr><td colspan=10>Помилка сервера при запиті<br>".$UpdateGQuery.
								" : ".mysqli_error($conn)."</td></tr></table>");
		} else {
?>
...
?>
	<tr><th>Код</th><th style="width: 80px;">Шифр інституту</th>
		<th style="width: 40px;">Курс</th><th style="width: 110px;">Шифр академгрупи</th>
		<th style="width: 135px;">Реєстровий номер РНП</th>
		<th style="width: 50px;">Кільк. студ.</th><th>Змінити академгрупу</th><th>До видалення</th></tr>
</thead>
<tbody style="height: 810px;">
<?php
// Завантажити перелік груп
	$query_result = mysqli_query($conn,  $GQuery) or 
...
?>
		...
		<td>
<?php 
		if ($TrueAdmin) echo paramCheker("cbxg".$query_row['id'], $_POST['cbxg'.$query_row['id']], 
...
?>
			<div><?php 
				...
				<input type="textbox" name="tbxc<?php echo $query_row['id']; ?>" 
						...
						value="<?php echo $query_row['num_kurs']; ?>" /><br>
				Шифр академгрупи: 
...
?>
		...
		<td><?php 
		if ($TrueAdmin) echo paramCheker("delg".$query_row['id'],$_POST['delg'.$query_row['id']],"",""); ?></td>
	</tr>
<?php 
		$icnt++;
	}
...
<tfoot>
	<tr>
		<td colspan=5 style="width: 891px; text-align: right;">Кількість академгруп: <?php echo bold($icnt); ?></td>
	<td><?php 
if ($TrueAdmin) { ?>
